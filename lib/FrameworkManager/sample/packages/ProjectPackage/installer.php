...
// プロジェクトインストーラー (フレームワーク付属のインストーラを利用しています)
...
define("PROJECT_CONFIG_PREFIX", $projectpkgName);
define("PROJECT_PACKAGE_NAME", PROJECT_CONFIG_PREFIX . "Package");
define("PROJECT_DB_NAME", "project");
define("PROJECT_DB_USER", "project");
define("PROJECT_DB_PASS", "projectpass");
...
// XXX インストーラによって変更されます。行を固定して下さい！
...
// XXX インストーラによって変更されます。行を固定して下さい！
$projpkgPath = dirname(dirname(dirname(__FILE__)))."/lib/".PROJECT_PACKAGE_NAME;
// org
//$projpkgPath = dirname(__FILE__)."/lib/FrameworkManager";
...
$(document).ready(function(){
			...
			if(stepApply <= maxStep3apply){
				...
				if(1 == stepApply){
					...
					if(1 > $("#input-fwmpath").val().length){
						$("#page" + argStep + " .errormsg").show();
						$("#page" + argStep + " .errormsg").text("(!!!)<?php echo PROJECT_NAME; ?>本体の現在のパスを入力して下さい。");
						// 終了
						return;
					}
					...
					if(1 > $("#input-newfwmpath").val().length){
						$("#page" + argStep + " .errormsg").show();
						$("#page" + argStep + " .errormsg").text("(!!!)<?php echo PROJECT_NAME; ?>本体の移動先のパスを入力して下さい。");
						// 終了
						return;
					}
					...
					inPageLocation("#pagetop");
					return;
					/* プロジェクトのインストーラはここまでとする！！ */
					// applyステップ8のバリデート
					if(1 > $("#input-fwmdocpath").val().length){
						$("#page" + argStep + " .errormsg").show();
						$("#page" + argStep + " .errormsg").text("(!!!)<?php echo PROJECT_NAME; ?>の公開ディレクトリの現在のパスを入力して下さい。");
						// 終了
						return;
					}
					...
					if(1 > $("#input-fwmbaseurl").val().length){
						$("#page" + argStep + " .errormsg").show();
						$("#page" + argStep + " .errormsg").text("(!!!)<?php echo PROJECT_NAME; ?>の公開ディレクトリのURLを入力して下さい。");
						// 終了
						return;
					}
					if(1 > $("#input-newfwmdocpath").val().length){
						$("#page" + argStep + " .errormsg").show();
						$("#page" + argStep + " .errormsg").text("(!!!)<?php echo PROJECT_NAME; ?>の公開ディレクトリの移動先のパスを入力して下さい。");
						// 終了
						return;
					}
	...
	$("#input-fwmpath").val("<?php echo $projpkgPath; ?>");
	$("#input-mysqluser").val("root");
	$("#input-mysqlpass").val("root");
	$("#input-fwmdbuser").val("<?php echo PROJECT_DB_USER; ?>");
	$("#input-fwmdbpass").val("<?php echo PROJECT_DB_PASS; ?>");
	$("#input-fwmdb").val("<?php echo PROJECT_DB_NAME; ?>");
...
<body id="pagetop">
	<h1 id="title" class="title"><?php echo PROJECT_NAME; ?></h1>
	<section id="page0" class="page_box">
		<article class="page">
			...
			<div id="page1_body" class="page_body">
				<p>
					<strong class="orange"><?php echo PROJECT_NAME; ?>のインストールを開始します。</strong><br>
					<br>
					...
					<br>
					<small>・フレームワークの各種パスの確認と変更(必須)</small>
					<br>
					<small>・<?php echo PROJECT_NAME; ?>本体のインストール(必須)</small>
					<br>
					<small>・<?php echo PROJECT_NAME; ?>管理機能のアクセス権設定(任意:※準備中)</small>
					<br>
					<br>
					<br>
					<strong>「インストールを開始する」を押して、インストールを開始しましょう！</strong>
					<br>
				</p>
			</div>
		</article>
	...
	<section id="page2" class="page_box">
		<article class="page">
			<h3 id="page2_title" class="page_title"><strong class="orange">STEP:2</strong><br>フレームワークの各種パスの確認と変更(必須)</h3>
			...
			<div id="page2_body2" class="page_body">
				...
				<p>
					...
					<br>
					<strong>”GenericPackage”</strong>と呼びます。
					<br>
					また、”VendorPackage”には、<?php echo PROJECT_NAME; ?>が利用している各種の外部ライブラリが格納されています。
					<br>
	...
	<section id="page3" class="page_box">
		<article class="page">
			<h3 id="page3_title" class="page_title"><strong class="orange">STEP:3</strong><br><?php echo PROJECT_NAME; ?>本体のインストール(必須)</h3>
			<div id="page3_body1" class="page_body">
				<p>
					<strong class="orange">先ず、<?php echo PROJECT_NAME; ?>本体の現在のパスを確認させて下さい。</strong>
					<br>
					<br>
					<?php echo PROJECT_NAME; ?>本体のインストールは”<?php echo PROJECT_PACKAGE_NAME; ?>”と言うディレクトリ名で
					<br>
					ダウンロード直後は
					<br>
					<strong class="orange"><?php echo $projpkgPath; ?></strong>
					<br>
					に、あるハズです。
					<br>
					<br>
			...
			<div id="page3_body2" class="page_body">
				<p>
					<strong class="green"><?php echo PROJECT_NAME; ?>本体は指定のパスで確認出来ました！</strong>
					<br>
					<br>
					...
					<br>
					<br>
					<strong class="orange"><?php echo PROJECT_NAME; ?>本体を、指定の任意のパスに移動する事が出来ます。</strong>
					<br>
					<br>
			...
			<div id="page3_body4" class="page_body">
				<p>
					<strong class="green"><?php echo PROJECT_NAME; ?>本体のパスの設定が無事に完了しました！</strong>
					<br>
					<br>
					<strong>下の移動ログを確認し、「設定」ボタンを押して下さい。</strong>
					<br>
			...
			<div id="page3_body5" class="page_body">
				<p>
					<strong class="orange"><?php echo PROJECT_NAME; ?>用のデータベースを作成します。</strong>
					<br>
					<br>
					<strong class="red">
			...
			<div id="page3_body6" class="page_body">
				<p>
					<strong class="green"><?php echo PROJECT_NAME; ?>用のデータベースを作成しました！</strong>
					<br>
					<br>
					<strong class="orange">
						<?php echo PROJECT_NAME; ?>用のデータベースへテスト接続と
						<?php echo PROJECT_NAME; ?>の設定ファイルの更新を行います。
					</strong>
					<br>
					<br>
					<strong>作成した<?php echo PROJECT_NAME; ?>用のデータベースの接続情報を入力して「設定」ボタンを押して下さい。</strong>
					<br>
					<br>
			...
			<div id="page3_body7" class="page_body">
				<p>
					<strong class="green">
						<?php echo PROJECT_NAME; ?>用のデータベースへのテスト接続に成功しました！
						<?php echo PROJECT_NAME; ?>の設定ファイルを更新しました！
					</strong>
					<br>
					<strong class="orange">
						<?php echo PROJECT_NAME; ?>が利用する、データベーステーブルの作成を行います。
					</strong>
					<br>
			...
			<div id="page3_body8" class="page_body">
				<p>
					<strong class="green"><?php echo PROJECT_NAME; ?>用のデータベースへテーブルを追加しました！</strong>
					<br>
					<br>
					<strong>「設定」ボタンを押して、設定を完了して下さい。</strong>
					<br>
					<br>
<!--
					<strong class="orange">
...
-->
			...
			<div id="page3_body9" class="page_body">
				<p>
					...
					<br>
					<br>
					<strong class="green"><?php echo PROJECT_NAME; ?>本体のインストールがが完了しました。</strong>
					<br>
					<strong class="green"><?php echo PROJECT_NAME; ?>にアクセスして、動作確認をしてみましょう！！</strong>
					<br>
					<br>
					<br>
					<strong class="orange">
						※次以降は<?php echo PROJECT_NAME; ?>管理機能のアクセス権の確認・設定(任意:※準備中)が行えます。
						<?php echo PROJECT_NAME; ?>管理機能のアクセス権の確認・設定(任意:※準備中)を行う場合は、「次のステップへ」ボタンを押して下さい。
					</strong>
					<br>
					<br>
<!--
...
-->
			...
			<div id="page3_body12" class="page_body">
				<p>
					...
					<br>
					<br>
					<strong>フレームワークマネージャーの詳しい利用方法は、Webサイト「<a href="http://saimushi.github.io/UNICORN/" target="_blank">準備中</a>」を確認して下さい。</strong>
				</p>
			</div>
			<div class="page_body">
				<pre class="hlog"></pre>
	...
	<section id="page4" class="page_box">
		<article class="page">
			<h3 id="page4_title" class="page_title"><strong class="orange">STEP:4</strong><br><?php echo PROJECT_NAME; ?>管理機能のアクセス権の設定(任意:※準備中)</h3>
			<div id="page4_body1" class="page_body">
				<p>
					<strong class="orange">準備中</strong>
					<br>
					<br>
					<?php echo PROJECT_NAME; ?>管理機能管理機能のhttps設定や、接続ネットワークの限定などの設定が行えるようになる予定です。
					<br>
					<br>
				</p>
			</div>
	...
	<nav id="navigatior" class="navigation_box">
		<div class="navigator">
			...
			<div id="navigatestep2" class="navigate_step leftfloat">
				...
				<div class="navigate_body">フレームワークの各種パスの確認と変更(必須)</div>
			</div>
			<div id="navigatestep3" class="navigate_step leftfloat">
				<p class="navigate_header">STEP:3</p>
				<div class="navigate_body"><?php echo PROJECT_NAME; ?>本体のインストール(必須)</div>
			</div>
			<div id="navigatestep4" class="navigate_step leftfloat">
				<p class="navigate_header">STEP:4</p>
				<div class="navigate_body"><?php echo PROJECT_NAME; ?>管理機能のアクセス権の設定(任意)</small></div>
			</div>
		</div>
	</nav>
...
// 				installerlog(var_export($confPaths, true));
	...
	// ステップ3フレームワークマネージャーのインストール
		...
		if(isset($_GET["apply"]) && 1 === (int)$_GET["apply"]) {
			...
			if(false === is_file($path."/core/".PROJECT_CONFIG_PREFIX.".config.xml")){
				// 存在を確認出来ず
			...
			// このファイル上の26行目の$projpkgPathを強制的に書き換える
			$targetLineNum = 26;
			...
			$replaceStr = "\$projpkgPath = ".$pathCode.".\"/".$newfwmLinkPath."\";";
			...
			if (!$connect) {
				exit("{\"ok\":false,\"error\":\"".PROJECT_NAME."用のデータベースへの接続に失敗しました。 \\n " . mysqli_connect_error() . " \\n 考えられる理由: \\n 1.指定されたユーザー・パスワード・データベース名が間違っています。 \\n 2.指定されたユーザー・パスワードに".PROJECT_NAME."用のデータベースへのアクセス権限が無いかも知れません。\"}");
			}
			if (!mysqli_set_charset($connect, "utf8")) {
				exit("{\"ok\":false,\"error\":\"mysqli_set_charsetの実行に失敗しました。 \\n " . mysqli_error($connect) . " \\n 考えられる理由: \\n 1.指定された設定情報にmysqli_set_charsetの実行権限が無いかも知れません。\"}");
			}
			...
			$fwmConfXMLPath = $path."/core/".PROJECT_CONFIG_PREFIX.".config.xml";
			if(false === is_file($fwmConfXMLPath)){
				// create文が見つからないエラー！
				exit("{\"ok\":false,\"error\":\"" . $path."/core/".PROJECT_CONFIG_PREFIX.".config.xmlを見つけられませんでした。\"}");
			}
			...
			exit("{\"ok\":true, \"fwmdocpath\":\"" . dirname(dirname($path))."/webdocs" . "\"}");
		}
			...
			$fwmpkgName = basename($projpkgPath);
			$targetLineNum1 = 4;
			// 5行目、フレームワークパスの特定処理
			$targetLineNum2 = 5;
	...
	if(isset($argv[1])){
		...
		// 以下Cli処理
		echo "\n";
		echo "--- ようこそ、".PROJECT_NAME." installer江 ---\n";
		echo "\n";
		echo "すみません、残念ながらCLIでのインストーラーは現在提供していません。\n";
		echo "提供次期は未定です。\n";
		echo "\n";
		echo "Webブラウザベースでのインストーラーを利用して下さい。\n";
		echo "http://[あなたのドメイン]/[あなたのフレームワークを配置したworkspaceディレクトリパス]/installer.php\n";
