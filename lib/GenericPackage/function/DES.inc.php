...
class DES {
	...
	public function encrypt($message, $key) {
		return base64_encode($this->execute($key, $message, 1, 0, null, 2));
	}
	...
	public function decrypt($message, $key) {
		return $this->execute($key, base64_decode($message), 0, 0, null, 2);
	}
	 ...
	 * @param object $mode
	 * @param object $iv
	 * @param object $padding[optional]
	 */
	public function execute($key, $message, $encrypt, $mode, $iv, $padding = 2) {
		...
		if ($iterations == 3) {
			if ($encrypt) {
				$looping = array (0, 32, 2);
			}
			else {
				$looping = array (30, -2, -2);
			}
		}
		else {
			if ($encrypt) {
				$looping = array (0, 32, 2, 62, 30, -2, 64, 96, 2);
			}
			else {
				$looping = array (94, 62, -2, 32, 64, 2, 30, -2, -2);
			}
		}
